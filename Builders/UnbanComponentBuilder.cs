...
public sealed class UnbanComponentBuilder
{
    private static readonly Color AccentColor = new(8, 4, 4);

    public MessageComponent BuildPrompt(PendingUnbanRequest request)
    {
        var content =
            $"> **Target:** `{EscapeInlineCode(request.TargetName)}`\n" +
            $"> **Target ID:** `{request.TargetId}`\n" +
            $"> **Reason:** `{EscapeInlineCode(request.Reason)}`\n" +
            "> This action will remove the user's ban from this server.";

        return BuildContainer(
            BuildHeader("Confirm Unban", content, request.TargetAvatarUrl, request.TargetName),
    ...
    public MessageComponent BuildSuccess(PendingUnbanRequest request)
    {
        return BuildNotice(
            "Unban Complete",
            $"`{EscapeInlineCode(request.TargetName)}` has been unbanned. Reason: `{EscapeInlineCode(request.Reason)}`");
    }

    public MessageComponent BuildCancelled(PendingUnbanRequest request)
    {
        return BuildNotice(
            "Unban Cancelled",
            $"Unban request for `{EscapeInlineCode(request.TargetName)}` was cancelled.");
    }
        ...
        return new ActionRowBuilder()
            .WithButton(
                ButtonBuilder.CreateSuccessButton(
                    "Confirm Unban",
                    UnbanComponentIds.Create(UnbanAction.Confirm, requestId)))
            .WithButton(
                ButtonBuilder.CreateSecondaryButton(
                    "Cancel",
                    UnbanComponentIds.Create(UnbanAction.Cancel, requestId)));
    }
